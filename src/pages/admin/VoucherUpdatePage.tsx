...
export const VoucherUpdatePage = () => {
  ...
  const { data: voucher, isLoading: isVoucherLoading, isError: isVoucherError } = useVoucher(id!);
  ...
  const { mutate: updateVoucher, isPending } = useUpdateVoucher(id!, {
    onSuccess: () => {
      message.success('Voucher berhasil diperbarui');
      navigate('/super-admin/voucher');
    },
    onError: (err: AxiosError<{ message?: string }>) => {
      message.error(err.response?.data?.message || 'Gagal memperbarui voucher');
    }
  });

  const onFinish = (values: any) => {
    const payload: Partial<CreateVoucherRequest> = {
      name: values.name,
      code: values.code,
      type: values.type,
      value: values.value,
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl mx-auto w-full">
      <div className="flex items-center gap-4">
        ...
        />
        <Title level={3} className="m-0!">Edit Voucher</Title>
      </div>
